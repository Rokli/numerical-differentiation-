...
class PassingMethod {
public:
    vector<vector<float>> Start(vector<vector<float>> matrix, vector<float> freeValues){
    ...
    vector<vector<float>> ReverseStroke(vector<vector<float>> matrix, vector<float> freeValues){
        double sum = 0;
